...
pkgdatadir = $(datadir)/giro
pkgincludedir = $(includedir)/giro
pkglibdir = $(libdir)/giro
pkglibexecdir = $(libexecdir)/giro
...
CONFIG_HEADER = $(top_builddir)/src/config/giro-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/giro-config.h.in \
...
ACLOCAL = ${SHELL} /home/chunche/giro/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/chunche/giro/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/chunche/giro/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/chunche/giro/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /home/chunche/giro/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = giro
PACKAGE_BUGREPORT = www.giro.org
PACKAGE_NAME = giro Core
PACKAGE_STRING = giro Core 2.3.0
PACKAGE_TARNAME = giro
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /home/chunche/giro
abs_srcdir = /home/chunche/giro
abs_top_builddir = /home/chunche/giro
abs_top_srcdir = /home/chunche/giro
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/chunche/giro/build-aux/install-sh
...
SUBDIRS = src
GZIP_ENV = "-9n"
BITCOIND_BIN = $(top_builddir)/src/girod$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/giro-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/giro-cli$(EXEEXT)
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_giro_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_giro_coverage.info test_giro.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/giro-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/giro-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/giro-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/giro-config.h
$(top_srcdir)/src/config/giro-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/giro-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/giro-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/giro-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/giro-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_giro.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_giro -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_giro_filtered.info: test_giro.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_giro_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_giro_coverage.info: baseline_filtered_combined.info test_giro_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_giro_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_giro_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_giro_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_giro.coverage/.dirstamp:  test_giro_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_giro.coverage/.dirstamp total.coverage/.dirstamp

# giro: disable the outdated comparison tool test
...
clean-local:
	rm -rf test_giro.coverage/ total.coverage/ $(OSX_APP)
