...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if(prev->nHeight <= 9) { txNew.vout[0].nValue = 112500 * COIN;} // Premine 112,500 * 10 = 1,125,000 giro 1.5%
    if (prev->nHeight > 9 && prev->nHeight <= 300) { txNew.vout[0].nValue = 1 * COIN;} // Block 9 to 300 pay 1 to prevent instamine
    if(prev->nHeight > 300) { txNew.vout[0].nValue = 70 * COIN;} // Block 300 or higher pay 70
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("giroMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("giroMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("giroMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("giro-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running giroMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
